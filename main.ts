...
window.addEventListener('DOMContentLoaded', () => {
    // 1. DOM要素を安全に取得
    const vInput = document.getElementById('voltage') as HTMLInputElement;
    const iInput = document.getElementById('current') as HTMLInputElement;
    const rInput = document.getElementById('resistance') as HTMLInputElement;

    const outV = document.getElementById('out-v') as HTMLSpanElement;
    const outI = document.getElementById('out-i') as HTMLSpanElement;
    const outR = document.getElementById('out-r') as HTMLSpanElement;
    const outP = document.getElementById('out-p') as HTMLSpanElement;
    const msgBox = document.getElementById('msgBox') as HTMLDivElement;

    const guiV = document.getElementById('gui-v') as HTMLSpanElement;
    const guiI = document.getElementById('gui-i') as HTMLSpanElement;
    const guiR = document.getElementById('gui-r') as HTMLSpanElement;
    const electron = document.getElementById('electron-dot') as HTMLDivElement;
    ...
    function updateUI(v: number, i: number, r: number, p: number, msg: string): void {
    ...
    function resetOutputs(): void {
        vInput.placeholder = "例: 5"; iInput.placeholder = "例: 0.02"; rInput.placeholder = "例: 250";
        outV.textContent = "--"; outI.textContent = "--"; outR.textContent = "--"; outP.textContent = "--";
        guiV.textContent = "-- V"; guiI.textContent = "-- A"; guiR.textContent = "-- Ω";
        msgBox.textContent = "";
    }

    function startElectronAnimation(current: number): void {
        stopElectronAnimation();
        ...
        function animate() {
            ...
            if (electronProgress < 25) {
                x = 50 + (electronProgress / 25) * 390; y = 30;
            } else if (electronProgress < 50) {
                x = 440; y = 30 + ((electronProgress - 25) / 25) * 96;
            } else if (electronProgress < 75) {
                x = 440 - ((electronProgress - 50) / 25) * 390; y = 130;
            } else {
                x = 50; y = 130 - ((electronProgress - 75) / 25) * 96;
            }
    ...
    function stopElectronAnimation(): void {
        if (animationId !== null) {
